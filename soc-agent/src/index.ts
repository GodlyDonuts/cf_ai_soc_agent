...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    console.log(`[Handshake] Incoming Request: ${request.method} ${request.url}`);
    console.log(`[Handshake] Upgrade Header (Raw): "${upgradeHeader}"`);
    console.log(`[Handshake] Is WebSocket: ${isWebSocket}`);

    if (isWebSocket) {
      const ticketId = url.searchParams.get('ticketId') ?? 'default';
      console.log(`[Handshake] WebSocket Upgrade detected. ticketId=${ticketId}`);
      try {
        const id = env.INVESTIGATION_TICKET.idFromName(ticketId);
        return env.INVESTIGATION_TICKET.get(id).fetch(request);
      } catch (err) {
        console.error(`[Handshake] Error getting Durable Object:`, err);
    ...
    const html = `
      ...
      <body>
          <div class="card">
              ...
              <p>The Autonomous Edge Security Analyst is operational. This worker coordinates real-time forensic investigations via WebSockets.</p>
              <div class="links">
              ...
              <div class="footer">v1.2.0 | cloudflare-edge-ai</div>
          </div>
      </body>
      </html>
    `;
